...
function configureSendbox(config) {
  ...
  if (config.databaseUrl) {
    _sql = _serverless.neon.call(void 0, config.databaseUrl);
  }
}
function getSQL() {
  if (_sql) return _sql;
  ...
  return _serverless.neon.call(void 0, url);
}
...
function getResend() {
  if (!_client) {
    _client = new (0, _resend.Resend)(getResendApiKey());
  }
  return _client;
}
...
async function createIdentity(data) {
  ...
  const rows = await sql`
    ...
    VALUES (${data.name}, ${data.email}, ${_nullishCoalesce(data.role, () => ( null))}, ${_nullishCoalesce(data.avatar_url, () => ( null))})
    RETURNING *
  `;
  return rows[0];
}
...
async function createThread(data) {
  ...
  const rows = await sql`
    ...
    VALUES (
      ${data.subject},
      ${data.identity_id},
      ${_nullishCoalesce(data.status, () => ( "open"))},
      ${_nullishCoalesce(data.labels, () => ( []))}
    )
    RETURNING *
  `;
  return rows[0];
...
async function createMessage(data) {
  ...
  const rows = await sql`
    ...
    VALUES (
      ${data.thread_id},
      ${_nullishCoalesce(data.resend_id, () => ( null))},
      ${data.direction},
      ${data.from_email},
      ${_nullishCoalesce(data.from_name, () => ( null))},
      ${data.to_email},
      ${_nullishCoalesce(data.cc, () => ( []))},
      ${_nullishCoalesce(data.subject, () => ( null))},
      ${_nullishCoalesce(data.body_html, () => ( null))},
      ${_nullishCoalesce(data.body_text, () => ( null))},
      ${JSON.stringify(_nullishCoalesce(data.attachments, () => ( [])))}
    )
    RETURNING *
  `;
  await sql`
...
async function processInboundEmail(payload) {
  ...
  const attachmentsMeta = (_nullishCoalesce(payload.attachments, () => ( []))).map((a) => ({
    filename: a.filename,
  ...
  const message = await createMessage({
    ...
    from_name: _nullishCoalesce(_optionalChain([payload, 'access', _ => _.from, 'access', _2 => _2.match, 'call', _3 => _3(/^([^<]+)</), 'optionalAccess', _4 => _4[1], 'optionalAccess', _5 => _5.trim, 'call', _6 => _6()]), () => ( void 0)),
    to_email: identity.email,
    subject: payload.subject,
    body_html: _nullishCoalesce(payload.html, () => ( void 0)),
    body_text: _nullishCoalesce(payload.text, () => ( void 0)),
...
exports.configureSendbox = configureSendbox; exports.getResend = getResend; exports.getIdentities = getIdentities; exports.createIdentity = createIdentity; exports.deleteIdentity = deleteIdentity; exports.getThreads = getThreads; exports.createThread = createThread; exports.getThreadsWithPreview = getThreadsWithPreview; exports.getThread = getThread; exports.getThreadWithIdentity = getThreadWithIdentity; exports.getThreadMessages = getThreadMessages; exports.createMessage = createMessage; exports.normalizeSubject = normalizeSubject; exports.parseEmailAddress = parseEmailAddress; exports.matchIdentity = matchIdentity; exports.matchThread = matchThread; exports.processInboundEmail = processInboundEmail;
